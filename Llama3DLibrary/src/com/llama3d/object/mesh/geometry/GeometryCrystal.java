...
public class GeometryCrystal extends Geometry {

	public GeometryCrystal() {
		this(5);
	}

	public GeometryCrystal(int segments) {
		...
		for (int i = 0; i < segments; i++) {
			...
			v[1].normal(2f * (float) Math.cos(Math.toRadians(step * i)), 1f, 2f * (float) Math.sin(Math.toRadians(step * i)));
			v[2].normal(2f * (float) Math.cos(Math.toRadians(step * (i + 1f))), 1f, 2f * (float) Math.sin(Math.toRadians(step * (i + 1f))));
			v[3].normal(0, -1, 0);
			v[4].normal(0, -1, 0);
			v[5].normal(0, -1, 0);
