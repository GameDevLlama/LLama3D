...
public class GeometryTorus extends Geometry {
	public GeometryTorus() {
		this(1f, 2f, 20, 20);
	}

	public GeometryTorus(float innerRadius, float outerRadius) {
		this(innerRadius, outerRadius, 20, 20);
	}

	public GeometryTorus(float innerRadius, float outerRadius, int rings, int segments) {
		...
		for (float segment1 = 0; segment1 <= 360.0f; segment1 += step[0]) {
			for (float segment2 = 0; segment2 < 360.0; segment2 += step[1]) {
				...
				position[2] = (float) Math.cos(Math.toRadians(segment2)); // x0
				position[3] = (float) Math.cos(Math.toRadians(segment2 + step[1])); // x1
				position[4] = (float) Math.sin(Math.toRadians(segment2)); // z0
				position[5] = (float) Math.sin(Math.toRadians(segment2 + step[1])); // z1
